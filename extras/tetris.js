...
Board.prototype.update_points = function (add_points) {
	this.points += add_points
	let points_label = document.getElementById("puntuacion")
	points_label.innerHTML = this.points
	let ganar = false
	let dificultad = document.getElementById("dificultad")
	if (this.points >= 30 && dificultad.value === "facil") {
			ganar = true
	} else if (this.points >= 40 && dificultad.value === "normal") {
			ganar = true
	} else if(this.points >= 50 && dificultad.value === "dificil") {
			ganar = true;
		}
		if (ganar) {
			clearInterval(timer)
			setTimeout(ganado, 1000)
		}
}
...
Tetris.LEVELS = ['Fácil', 'Normal', 'Difícil'];
...
Tetris.prototype.init = function(){
	...
	initialize_listeners(this)
	let tetris = this;
	let objetivo = document.getElementById("objetivo")
	let puntuacion = document.getElementById("puntuacion")
	// Obtener una nueva pieza al azar y asignarla como pieza actual
	this.previous_shape = this.create_new_shape()
	draw_previous_shape(this.previous_shape)
...
function initialize_listeners(tetris) {
	let dificultad = document.getElementById("dificultad")
	dificultad.addEventListener('change', function () {
		...
		if (dificultad.value === "facil") {
			interval = 1000;
			objetivo.innerHTML = 30
		} else if (dificultad.value === "normal") {
			interval = 700;
			objetivo.innerHTML = 40
		} else if (dificultad.value === "dificil") {
			interval = 500;
			objetivo.innerHTML = 50
		}
		timer = setInterval(function (tetris){
			tetris.do_move('Down')
		}, interval, tetris)
...
Tetris.prototype.key_pressed = function(e) {
	...
	if (80 === key) {
		if (!pause) {
			...
			dificultad.dispatchEvent(event);
		}
	}
	else if (!pause) {
	...
	if (direction === 'Left' || direction === 'Down' || direction === 'Right') {
		...
		if (move) {
			...
			if (direction === 'Down') {
				...
				this.current_shape = this.previous_shape
				this.previous_shape = this.create_new_shape()
				draw_previous_shape(this.previous_shape)
...
function draw_previous_shape(shape) {
	let previsualization = document.getElementById("next_shape")
	var ctx2 = previsualization.getContext("2d");
	ctx2.fillStyle = 'white';
	ctx2.fillRect(0,0,previsualization.width,previsualization.height);
	let old_ctx = ctx
	ctx = ctx2
	shape.draw()
	ctx = old_ctx
